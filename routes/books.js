  ...
  try {
    ...
    if (authors.constructor == Array) {
      const authorsPromises = authors.map(async (authorId) => {
        ...
        if (book.authors.includes(author._id))
          throw new Error(`${author.username} is already an author`);
        return author._id;
      });
...
books.put("/update-book", upload, async (req, res) => {
  ...
  if (!book)
    return res
      .status(404)
      .json({ error: "Book not found, It might have been deleted" });
    ...
    if (authors.constructor == Array) {
      const authorsPromises = authors.map(async (authorId) => {
        if (!mongoose.isValidObjectId(authorId))
          throw new Error(
            `Invalid ObjectId for user ${authorId} in array of authors`
          );
        const author = await User.findById(authorId);
        if (!author) throw new Error(`${authorID} not found`);
        if (book.authors.includes(author._id))
      ...
      try {
        if (!mongoose.isValidObjectId(authors))
          throw new Error(`Invalid ObjectId for user ${authors}`);
        const author = await User.findById(authors);
        book.authors.push(author._id);
      } catch (error) {
        return res.status(500).json({ error: error.message });
  ...
  try {
    ...
    return res
      .status(200)
      .json({ message: "Successfully updated book", book: updatedBook });
  } catch (error) {
    return res.status(500).json({ error: error.message });
  }
});
